...
describe('useTabPanel', () => {
  ...
  it('initialize 恢复 session iframe tab 时清洗非法 url', async () => {
    ...
    expect(panel.tabs.value.find((tab) => tab.id === 'restored-tab')?.url).toBe('about:blank')
  ...
  it('从路由 __src 创建 iframe tab 时清洗非法 url', async () => {
    ...
    expect(panel.tabs.value.find((tab) => tab.id === 'iframe-tab')?.url).toBe('about:blank')
  ...
  it('从路由 malformed __src 创建 iframe tab 时降级为 about:blank', async () => {
    ...
    expect(panel.tabs.value.find((tab) => tab.id === 'iframe-malformed-src')?.url).toBe(
      'about:blank'
    )
  ...
  it('active 路由导航失败时回滚 active 状态与 session', async () => {
    ...
    expect(panel.tabs.value.find((tab) => tab.id === 'tab-1')?.active).toBe(true)
    expect(panel.tabs.value.find((tab) => tab.id === 'tab-2')?.active).toBe(false)
